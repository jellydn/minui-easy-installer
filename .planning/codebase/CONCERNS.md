...
- Issue: Device profiles are defined in TWO separate places — `src/types/device.ts` (hardcoded array) and `src/types/device-install-map.json` (JSON data file). The `device.ts` file has 16 devices including "Miyoo Mini" and "miyoo355" that differ from the JSON mapping. The `device-install-map.ts` is a superset with more granular metadata (devicePaks, install actions, sharedBios). These two sources are not kept in sync and could drift.
...
- Issue: `is_update_available()` in `src-tauri/src/version.rs` (line 97-104) compares version strings using Rust's `>` operator, which is lexicographic. This works for date-based versions like "2024.12.25" but will break for non-date version formats (e.g., "0.12.0" vs "0.9.0" — "0.12.0" < "0.9.0" lexicographically).
- Files: `src-tauri/src/version.rs`
- Impact: Package version comparison in `src-tauri/src/package.rs` (line 92, `latest_version > installed_ver`) also uses string comparison. Third-party packages from the store may use semver-style versions where this silently gives wrong results.
...
- Issue: Multiple places silently discard errors using `let _ =`:
  ...
  - `src-tauri/src/install.rs:296` — Version metadata (`minui.txt`) write failure is silent
  - `src-tauri/src/extract.rs:156` — Unix file permission setting failure is silent
  - `src-tauri/src/lib.rs:67` — Progress event emission failure is silent
...
- Fix approach: Log warnings at minimum. For `minui.txt` write failures, surface an extras_warning-style message. For permission failures, at least log via `eprintln!`.
...
- Files: `src-tauri/src/wifi.rs` (line 52)
- Trigger: Any WiFi configuration save operation.
- Workaround: This is the MinUI expected format and cannot be changed without breaking device compatibility. However, the app should warn users that the SD card contains plaintext credentials if removed from the device.

**Potential WiFi Scan Output Parsing Failure on macOS 14.4+:**
- Symptoms: `scan_wifi_macos()` in `src-tauri/src/wifi.rs` (lines 142-166) tries the `airport` binary first (line 144-148), which Apple removed in macOS 14.4. The fallback (line 161) only returns the currently connected SSID, not a list of available networks.
...
- Symptoms: `list_removable_drives()` on macOS (line 15-66 of `drives.rs`) uses `df -k` and filters for paths starting with `/Volumes/`. This includes the built-in Macintosh HD if mounted there, plus any network drives.
...
- Symptoms: `handleUpdateAll` in `src/Home.tsx` (lines 253-324) only performs the MinUI base update. Package updates are stubbed out with a comment: "Package updates would be handled here. For now, we'll just show the message" (lines 297-301).
- Files: `src/Home.tsx`
...
- Risk: `reqwest::get(url)` in `src-tauri/src/download.rs` (line 48) has no timeout configured. A slow or hung server could block the app indefinitely.
- Files: `src-tauri/src/download.rs`
- Current mitigation: None. The Tauri IPC command is async, so the frontend can still interact, but the backend thread is blocked.
- Recommendations: Add a `reqwest::Client::builder().timeout(Duration::from_secs(120)).build()` for downloads. Add a separate, shorter timeout for the GitHub API metadata fetch.

**WiFi Password in Plaintext Without Warning:**
- Risk: The WiFi password is written to `wifi.txt` in plaintext. If the user removes the SD card and gives it to someone, or the card is lost, the WiFi password is exposed.
- Files: `src-tauri/src/wifi.rs`, `src/WifiWizard.tsx`
- Current mitigation: The `type="password"` input field hides the password during entry.
- Recommendations: Add a warning in the WiFi wizard UI that the password will be stored in plaintext on the SD card.

**Package Registry Not Validated Against Schema:**
- Risk: AGENTS.md states "Treat registry data as untrusted — validate schema before use." However, `convertStoreRegistry()` in `src/types/package.ts` (line 201) does minimal type checking. The `isStoreRegistry()` guard only checks for presence of `emu_paks` and `tool_paks` arrays but does not validate individual field types, URLs, or versions.
...
- Files: `src-tauri/src/install.rs` (line 138)
...
- Risk: The Tauri CSP in `src-tauri/tauri.conf.json` (line 24) includes `'unsafe-inline'` for `style-src`. This weakens XSS protection.
...
- Risk: Package installs from the store have `checksum: null` for all packages (see `src/types/store.json` and `src/types/package.ts` line 220). The `install_package` backend function accepts an optional checksum but it is never provided by the store data.
...
- Problem: `download_archive()` in `src-tauri/src/download.rs` (lines 59-64) calls `response.bytes().await` which loads the entire archive into memory before writing to disk. Large MinUI archives could be 500MB+.
- Files: `src-tauri/src/download.rs`
...
- Problem: `copy_dir_recursive()` and `copy_dir_contents()` in `src-tauri/src/fs_utils.rs` perform synchronous file I/O using `std::fs::copy`. Large installations copying thousands of files to SD cards (which are slow) will block the thread.
...
- Problem: The `useEffect` in `src/Home.tsx` (lines 56-111) calls `fetchMinUIRelease()` every time a drive is selected, which hits the GitHub API. Switching between drives triggers multiple API calls.
- Files: `src/Home.tsx`
...
**`reqwest` with `blocking` Feature Enabled But Not Used:**
- Risk: `Cargo.toml` includes `reqwest = { version = "0.12", features = ["json", "blocking"] }` but no code uses the blocking client — only the async client is used via `reqwest::get()`. This unnecessarily increases binary size and compile time.
- Files: `src-tauri/Cargo.toml`
- Impact: Larger binary, slower compilation.
- Migration plan: Remove `"blocking"` from the features list.

**`time` Crate Included But Not Used:**
- Risk: `Cargo.toml` includes `time = "0.3.36"` but no source file imports or uses it.
- Files: `src-tauri/Cargo.toml`
- Impact: Unnecessary dependency, increases attack surface and compile time.
- Migration plan: Remove from `Cargo.toml`.
...
**`windows-sys` Dependency Only for Future Windows Format Support:**
- Risk: `windows-sys = { version = "0.59", features = ["Win32_Storage_FileSystem"] }` is included but `format_drive` on Windows (line 153-155 of `drives.rs`) just returns an error string.
- Files: `src-tauri/Cargo.toml`, `src-tauri/src/drives.rs`
- Impact: Unused dependency on macOS builds (conditional), wasted compile time.
- Migration plan: Gate behind `cfg(target_os = "windows")` in Cargo.toml or remove until Windows formatting is implemented.
...
- Problem: If the base archive installs successfully but extras fail (line 277-280 of `install.rs`), the SD card is left in a partially-updated state with no way to roll back.
- Blocks: Users may end up with incompatible base + old extras combinations.

**No Checksum Verification for Store Packages:**
- Problem: All packages in `store.json` have `checksum: null`. The installer downloads and extracts packages without verifying integrity.
- Blocks: Cannot guarantee downloaded packages haven't been tampered with or corrupted during download.

**No Windows Drive Formatting:**
- Problem: `format_drive` on Windows (line 153-155 of `drives.rs`) returns a hardcoded error string. Windows users cannot format SD cards to FAT32.
- Blocks: Windows users must use external tools to format drives.

**No Linux Support:**
- Problem: Drive detection on Linux (line 300-304 of `drives.rs`) returns an error. No WiFi scanning fallback. `get_free_space` returns `None` on non-Unix.
